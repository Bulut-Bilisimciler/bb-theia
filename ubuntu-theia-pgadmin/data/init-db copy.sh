...
su - postgres -c "psql -U postgres -d test -c \"

SELECT Customers.CustomerName, sum(Quantity) FROM Customers
left join Orders on Orders.CustomerID = Customers.CustomerID
left join OrderDetails on OrderDetails.OrderID = Orders.OrderID
GROUP BY Customers.CustomerName
having sum(Quantity) >= 100;

SELECT Employees.FirstName||' '||Employees.LastName AS EmployeeFullName, Customers.CustomerName,  sum(Quantity) as TotalSales FROM Employees
left join Orders on Orders.EmployeeID = Employees.EmployeeID
left join OrderDetails on OrderDetails.OrderID = Orders.OrderID
left join Customers on Customers.CustomerID = Orders.CustomerID
WHERE 1=1
AND Customers.Country = 'Germany'
AND UPPER(Employees.FirstName||' '||Employees.LastName) LIKE '%O%'
AND UPPER(Employees.FirstName||' '||Employees.LastName) NOT LIKE '%O%O%'
Group By Employees.FirstName||' '||Employees.LastName, Customers.CustomerName
